...
const refreshToken = async (token: string) => {
  ...
  const session = await RefreshSession.findOne({ jti: String(jti) });
  ...
  const claimed = await RefreshSession.findOneAndUpdate(
    { jti: String(jti), used: false, revoked: false },
    { used: true },
    { new: true }
  );
  if (!claimed) {
...
const logout = async (token?: string) => {
  ...
  try {
    ...
    if (jti) {
      await RefreshSession.updateOne({ jti: String(jti) }, { revoked: true });
    }
    ...
    if (userId) {
      await User.updateOne({ _id: String(userId) }, { $inc: { tokenVersion: 1 } });
    }
  } catch (error) {
...
const changePassword = async (userPayload: any, payload: any) => {
  ...
  const user = await User.findById(String(userPayload._id));
